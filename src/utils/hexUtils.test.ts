...
describe("hexUtils", () => {
  describe("textToHex", () => {
    ...
    it("should convert Japanese text to hex correctly in gen3", () => {
      const text = "ポケモン";
      const language = "ja";
      const gen = "3";
      const expectedHex = "9F 59 73 7E";

      const result = textToHex(text, language, gen);
      expect(result).toBe(expectedHex);
    });

    it("should use the Japanese-based gen3 map regardless of UI language", () => {
      const text = "ポケモン";
      const language = "en";
      const gen = "3";
      const expectedHex = "9F 59 73 7E";
    ...
    it("should convert half-width alphanumerics via fullwidth aliases in gen3", () => {
      const text = "ABC abc 019!?";
      const language = "ja";
      const gen = "3";
      const expectedHex = "BB BC BD 00 D5 D6 D7 00 A1 A2 AA AB AC";

      const result = textToHex(text, language, gen);
      expect(result).toBe(expectedHex);
    });

    it("should convert a line break to the FF terminator in gen3", () => {
      const text = "あ\nい";
      const language = "ja";
      const gen = "3";
      const expectedHex = "01 FF 02";
  ...
  describe("hexToText", () => {
    ...
    it("should convert hex to Japanese text correctly in gen3", () => {
      const hex = "9F 59 73 7E";
      const language = "ja";
      const gen = "3";
      const expectedText = "ポケモン";

      const result = hexToText(hex, language, gen);
      expect(result).toBe(expectedText);
    });

    it("should convert gen3 fullwidth characters and control bytes", () => {
      const hex = "00 BB A1 FE FF";
      const language = "ja";
      const gen = "3";
      const expectedText = "　Ａ０\n\n";
  ...
  describe("hexToProgram (gen3, Thumb)", () => {
    it("should disassemble Thumb code correctly", () => {
      const hex = "01 20 10 B5 70 47 FF DF";
      const expectedProgram = "movs r0, #0x1\npush {r4, lr}\nbx lr\nswi #0xFF";

      const result = hexToProgram(hex, "3");
      expect(result).toBe(expectedProgram);
    });

    it("should combine bl halfword pairs", () => {
      const hex = "00 F0 00 FA";
      const expectedProgram = "bl #0x400";

      const result = hexToProgram(hex, "3");
      expect(result).toBe(expectedProgram);
    });

    it("should emit .hword for undefined instructions and .byte for a trailing byte", () => {
      const hex = "FF B1 01";
      const expectedProgram = ".hword 0xB1FF\n.byte 0x01";
  ...
  describe("hexToProgram (gen3, ARM)", () => {
    it("should disassemble ARM code correctly", () => {
      const hex = "01 03 A0 E3 1E FF 2F E1";
      const expectedProgram = "mov r0, #0x4000000\nbx lr";

      const result = hexToProgram(hex, "3", "arm");
      expect(result).toBe(expectedProgram);
    });

    it("should emit .word for undefined instructions", () => {
      const hex = "00 00 00 EE";
      const expectedProgram = ".word 0xEE000000";
  ...
  describe("programToHex (gen3, ARM)", () => {
    it("should assemble ARM code correctly", () => {
      const program = "mov r0, #0x4000000\nbx lr";
      const expectedHex = "01 03 A0 E3 1E FF 2F E1";

      const result = programToHex(program, "3", "arm");
      expect(result).toBe(expectedHex);
    });

    it("should default to Thumb when no CPU mode is given", () => {
      const program = "movs r0, #0x1";
      const expectedHex = "01 20";

      const result = programToHex(program, "3");
      expect(result).toBe(expectedHex);
    });
  });

  describe("programToHex (gen3, Thumb)", () => {
    it("should assemble Thumb code correctly", () => {
      const program = "movs r0, #0x1\npush {r4, lr}\nbx lr\nswi #0xFF";
      const expectedHex = "01 20 10 B5 70 47 FF DF";

      const result = programToHex(program, "3");
      expect(result).toBe(expectedHex);
    });

    it("should assemble bl into two halfwords", () => {
      const program = "bl #0x400";
      const expectedHex = "00 F0 00 FA";

      const result = programToHex(program, "3");
      expect(result).toBe(expectedHex);
    });

    it("should skip lines that cannot be assembled", () => {
      const program = "movs r0, #0x1\nnot an instruction\nbx lr";
      const expectedHex = "01 20 70 47";
  ...
  describe("hexToProgram", () => {
    it("should convert hex to program correctly", () => {
      ...
      const result = hexToProgram(hex, "1");
      expect(result).toBe(expectedProgram);
    });
  ...
  describe("programToHex", () => {
    it("should convert program to hex correctly", () => {
      ...
      const result = programToHex(program, "1");
      expect(result).toBe(expectedHex);
    });
